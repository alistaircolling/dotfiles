...
2. Try to fetch the PR directly by number. If the PR is not found (e.g. GraphQL returns null or NOT_FOUND), fall back to searching by branch name:
...
7. Then present each unresolved thread ONE AT A TIME in this format:
...
8. After displaying each comment, use AskUserQuestion to prompt the user:
   - Ask: `Continue to next comment? (y/n/skip to #N)`
