---
description: Review all changes on the current branch against its base
---
...
Find the associated Linear ticket for this PR. Check the branch name for a ticket identifier (e.g. `abc-1234`), and also check the PR description for any Linear links or ticket references.

If you find a ticket identifier, fetch the ticket details using the Linear GraphQL API:
```bash
curl -s -X POST https://api.linear.app/graphql \
  -H "Content-Type: application/json" \
  -H "Authorization: $LINEAR_API_KEY" \
  -d '{"query": "{ issue(id: \"ABC-XXXX\") { title description state { name } labels { nodes { name } } } }"}'
```

If `$LINEAR_API_KEY` is not set, note this and skip the Linear cross-check.
...
Compare all three and produce a clear **Requirements Consistency Report** section in your review. Flag the following with prominent warnings:
